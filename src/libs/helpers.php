...
function change_limit($err, $success)
{
    try {
        ...
        if (isset($_GET['formation'])) {
            $sql = "select * from delf_limite where formation = '$formation'";
            $query = db()->query($sql);
            if ($query->rowCount() > 0) {
                $update = "UPDATE delf_limite SET delf_lim = $newLimit WHERE formation = '$formation'";
                $update = db()->query($update);
            } else {
                $insert = "INSERT INTO delf_limite (formation, delf_lim) values ('$formation', '$newLimit')";
